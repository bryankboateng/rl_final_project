...
from utils import ReplayMemory, Transition, Batch
from actors_and_critics import build_network
from simulators.vec_env.vec_env import VecEnvBase
from simulators import BaseEnv


class BaseTraining(ABC):
  leaderboard: Union[PriorityQueue, np.ndarray] #leaderboard is maintained classlevel
  ...
  def __init__(self, cfg_solver, cfg_arch, seed: int):
    ...
    self.critics, self.actors = build_network(cfg_solver, cfg_arch, self.device, verbose=True) #Interesting uses cfg_solver as base cfg
    ...
    self.eval_period = int(self.cfg_solver.eval_period)  # Evaluates actors/critics every `eval_period` steps. Frequency of agent evaluation
    self.warmup_steps = int(self.cfg_solver.warmup_steps)  # Uses random actions before `warmup_steps`. Intial steps where random actions are used before the policy kicks in
    self.min_steps_b4_opt = int(self.cfg_solver.min_steps_b4_opt)  # Starts to optimize after `min_steps_b4_opt`. Number of steps before opt starts
    self.batch_size = int(cfg_solver.batch_size)
    self.max_model = int(cfg_solver.max_model) if cfg_solver.max_model is not None else None #Stores the max number of models (used for checkpoints)
  ...
  def learn(
    ...
    rollout_env = self.init_learn(env) #recieve an initialized vec_env 

    if self.eval_b4_learn and eval_callback is not None: # Eval the agent before learning starts
      self.eval(env, rollout_env, eval_callback, init_eval=True)

    start_learning = time.time()
    if self.num_envs == 1: # No need for vectorized env
      obsrv_all = rollout_env.reset(cast_torch=True, **reset_kwargs)[None] #how are we cast_torch what does it do and subscript with None
    else:
  ...
  def init_learn(self, env: BaseEnv) -> Union[BaseEnv, VecEnvBase]:
    ...
    if self.num_envs > 1: # Vectorized envs 
      ...
      # Make copies of current instance of global agent for exploration(not updates) to collect diverse batches
      self.agent_copy_list = [copy.deepcopy(env.agent) for _ in range(self.num_envs)] #why are agent policies placed on torch for more than one env
      for agent in self.agent_copy_list:
        agent.policy.net.to(torch.device(self.rollout_devce))
      rollout_env.set_attr("agent", self.agent_copy_list, value_batch=True) #what does value_batch do again?
    else:
