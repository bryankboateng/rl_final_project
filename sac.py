...
class SAC(BaseTraining):
    ...
    for env_idx, (done, info) in enumerate(zip(done_all, info_all)):
      ...
      if done:
        if self.num_envs == 1:
          ...
          obsrv_nxt_all[env_idx] = rollout_env.reset_one(index=env_idx) # Next obs for vector-parallelized envs
        g_x = info['g_x']
        if g_x < 0:
          self.cnt_safety_violation += 1 # Check violations across all envs
  ...
  def update_one(self, batch: Batch, timer: int) -> Tuple[float, float, float, float]:
    ...
    # Updates the critic.
    # put critic net and target in training mode
    # put actor net in eval
