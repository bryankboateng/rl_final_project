...
class Agent:
  ...
  def __init__(self, cfg, action_space: np.ndarray, env=None) -> None:
    # Import appropriate dynamics via cfg file specification
    # Dynamics object is instantiated with cfg and action_space
    ...
    elif cfg.dyn == "SpiritDstbPybullet":
      # Prevents from opening a pybullet simulator when we don't need to.
      from .dynamics.spirit_dstb_dynamics_pybullet import SpiritDstbDynamicsPybullet
      self.dyn = SpiritDstbDynamicsPybullet(cfg, action_space)
    elif cfg.dyn == "PointMass":
      from .dynamics.pm_dynamics_pybullet import PointMassPyBullet
      self.dyn = PointMassPyBullet(cfg, action_space)
    ...
    elif cfg.dyn == "HumanoidPybullet":
      from .dynamics.humanoid_dyn_pybullet import HumanoidDynamicsPybullet
      self.dyn = HumanoidDynamicsPybullet(cfg, action_space)
    else:
  ...
  def integrate_forward(
    ...
    # If control is none we acquire control by making a call to agent policy
    if control is None:
    ...
    # Refer to agent's dynamics to move forward
