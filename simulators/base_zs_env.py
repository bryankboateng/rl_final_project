...
class BaseZeroSumEnv(BaseEnv):
  ...
  def step(self, action: ActionZS,
    ...
    if "point_mass_identifier" in info:
      self.agent.dyn.capture_snapshot(done_signal=done, save_every=1, save_path=f"snapshots/")
  ...
  def simulate_one_trajectory(
      self, T_rollout: int, end_criterion: str, adversary: BasePolicy, reset_kwargs: Optional[Dict] = None,
    ...
    Args:
        ...
        adversary (BasePolicy):  a policy that maps current state and ctrl to adversarial ctrl (dstb).
    ...
    info = dict(
        obsrv_hist=np.array(obsrv_hist), action_hist=action_hist, plan_hist=plan_hist,
        reward_hist=np.array(reward_hist), step_hist=step_hist, shield_ind=shield_ind
    )
    return np.array(state_hist), result, info

  def simulate_trajectories(
      self, num_trajectories: int, T_rollout: int, end_criterion: str, adversary: BasePolicy,
