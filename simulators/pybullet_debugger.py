...
class pybulletDebug:
  ...
  def __init__(self, client, control=[0.0, 0.0, 0.0]):
  ...
  def cam_and_robotstates(self, boxId):
    robotPos, robotOrn = p.getBasePositionAndOrientation(boxId, physicsClientId=self.client)
    p.resetDebugVisualizerCamera(
        cameraDistance=self.cdist, cameraYaw=self.cyaw, cameraPitch=self.cpitch, cameraTargetPosition=robotPos,
