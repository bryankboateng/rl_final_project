...
class Go2DynamicsPybullet(BasePybulletDynamics):
  ...
  def __init__(self, config: Any, action_space: np.ndarray) -> None:
    ...
    if self.action_type == "increment":
      self.abduction_min = -0.8
      self.abduction_max = 0.8
      self.hip_min = -1.5
      self.hip_max = 1.5
      self.knee_min = -2.7
      self.knee_max = -0.85
    ...
    self.robot = None
    self.use_arrow_for_adversarial = False
        ...
        if state is not None:
          ...
          if len(self.state) == 32:  # no z
            ...
            if "initial_action" in kwargs.keys():
              ...
              p.stepSimulation(physicsClientId=self.client)
            break
          elif len(self.state) == 36:  # 32 + foot contact information, reset the same as len(self.state) == 32
            self.initial_height = 0.6

            self.initial_rotation = p.getQuaternionFromEuler([state[3], state[4], 0.0])  # yaw does not matter, there's no terrain anyway
            self.initial_joint_value = state[8:20]
            self.initial_joint_vel = state[20:]
            ...
            if self.use_arrow_for_adversarial:
              if self.adversarial_object is None:
                if self.gui and self.force > 0:
                  self.adversarial_object = Force(self.client)
          ...
          if height is None:
            if self.height_reset == "both":
              height = np.random.choice([0.5 + np.random.rand() * 0.2, 0.7], p=[0.5, 0.5])
            elif self.height_reset == "drop":
              height = 0.5 + np.random.rand() * 0.2
            else:
              height = 0.7
          ...
          if not is_rollout_shielding_reset:
            ...
            if self.initial_height_reset_type == "drop":
              ...
              self.robot.reset([0, 1.2, -2.4, 0, 1.2, -2.4, 0, 1.2, -2.4, 0, 1.2, -2.4])
              self.robot.apply_position([0, 1.2, -2.4, 0, 1.2, -2.4, 0, 1.2, -2.4, 0, 1.2, -2.4])
              p.stepSimulation(physicsClientId=self.client)
  ...
  def get_random_joint_value(self):
    if self.action_type == "increment":
      # return (0, 0.75, -1.8, 0, 0.75, -1.8, 0, 0.75, -1.8, 0, 0.75, -1.8) # check to see if there's mode collapse right from the beginning
      return (
          np.random.uniform(self.abduction_min, self.abduction_max), 
          0.6 + np.random.uniform(-0.5, 0.5),
          -1.5 + np.random.uniform(-0.5, 0.5), 
          np.random.uniform(self.abduction_min, self.abduction_max),
          0.6 + np.random.uniform(-0.5, 0.5), 
          -1.5 + np.random.uniform(-0.5, 0.5),
          np.random.uniform(self.abduction_min, self.abduction_max), 
      ...
      # return (
      #     0.3 + np.random.uniform(-0.5, 0.5),
      #     0.3 + np.random.uniform(-0.5, 0.5),
      #     -1.5 + np.random.uniform(-0.5, 0.5), 
      #     0.3 + np.random.uniform(-0.5, 0.5),
      #     0.7 + np.random.uniform(-0.5, 0.5), 
      #     -1.5 + np.random.uniform(-0.5, 0.5),
      #     -0.3 + np.random.uniform(-0.5, 0.5), 
      #     0.3 + np.random.uniform(-0.5, 0.5),
      #     -1.5 + np.random.uniform(-0.5, 0.5), 
  ...
  def integrate_forward(
    ...
    current_joint_pos = np.array(self.robot.get_joint_position(), dtype=np.float32)
    #! NOTE: if want to use control from Spirit, this current_joint_pos has to be Go2 (because we are mapping to Spirit in the .get_joint_position() funciton call

    ##############################
    #! do a mapping from Spirit control to Go2 control
    # go2_mapped_control = []
    # for i, j in enumerate(control):
    #   if i % 3 == 0:
    #     go2_mapped_control.append(j)
    #   elif i % 3 == 1:
    #     go2_mapped_control.append(0.5 * np.pi * np.cos(j))
    #   elif i % 3 == 2:
    #     go2_mapped_control.append(-0.5 * np.pi * (np.cos(j) + 1))
    # control = go2_mapped_control
    ##############################
    ...
    for i, j in enumerate(control):
      if i % 3 == 0:
        ...
        if self.abduction_min <= current_joint_pos[i] + increment <= self.abduction_max:
          clipped_control.append(increment)
        else:
          clipped_control.append(
              np.clip(current_joint_pos[i] + increment, self.abduction_min, self.abduction_max) - current_joint_pos[i]
          )
      elif i % 3 == 1:
        increment = np.clip(j, self.hip_increment_min, self.hip_increment_max)
        if self.hip_min <= current_joint_pos[i] + increment <= self.hip_max:
          clipped_control.append(increment)
        else:
          clipped_control.append(
              np.clip(current_joint_pos[i] + increment, self.hip_min, self.hip_max) - current_joint_pos[i]
          )
      elif i % 3 == 2:
        increment = np.clip(j, self.knee_increment_min, self.knee_increment_max)
        if self.knee_min <= current_joint_pos[i] + increment <= self.knee_max:
          clipped_control.append(increment)
        else:
          clipped_control.append(
              np.clip(current_joint_pos[i] + increment, self.knee_min, self.knee_max) - current_joint_pos[i]
          )
    ...
    if self.gui:
      if has_adversarial and not self.replace_adv_with_dr:
        if not self.use_arrow_for_adversarial:
