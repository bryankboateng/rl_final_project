...
def main(args):
  ...
  # with open("batch_1000_bust_1000.pkl", "rb") as f:
  #     data = pickle.load(f)

  # # Each key in data is a list of length N (e.g., 1000)
  # batch_index = 85
  # for key in data:
  #     print(f"{key}: {data[key][batch_index]}")
  ...
  for i in range(min_index, max_index + 1):
    ...
    for j in range(eval_horizon):
      ...
      a = {
    'ctrl': u.detach().cpu().numpy() if u.is_cuda else u.detach().numpy(),
    'dstb': adv.detach().cpu().numpy() if adv.is_cuda else adv.detach().numpy()
          }
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "--end_criterion", help="end criterion type", type=str, default="failure",
      choices=["default", "failure", "reach-avoid"]
  )
  parser.add_argument(
      "--rollout_end_criterion", help="rollout end criterion type", type=str, default="failure",
      choices=["default", "failure", "reach-avoid"]
  )
